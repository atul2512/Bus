...
ts.addWidget({
	...
	format: function(table, c, wo){
		if (c.parsers && !c.$table.hasClass('hasFilters')){
			var i, j, k, l, val, ff, x, xi, st, sel, str,sel1,
			...
			buildDefault = function(updating){
				...
				for (i = 0; i < cols; i++){
					...
					}else if ((t.hasClass('filter-select11') || wo.filter_functions && wo.filter_functions[i] === true) && !t.hasClass('filter-false')){
						if (!wo.filter_functions) { wo.filter_functions = {}; }
						wo.filter_functions[i] = true; // make sure this select gets processed by filter_functions
						buildSelect(i, updating, t.hasClass(wo.filter_onlyAvail));
					}
			...
			if (wo.filter_columnFilters !== false && $ths.filter('.filter-false').length !== $ths.length){
				...
				for (i = 0; i < cols; i++){
					...
					//sel1 = (wo.filter_functions && wo.filter_functions[i] && typeof wo.filter_functions[i] !== 'function') || $th.hasClass('filter-select11');
					...
					if (sel){
						...
						//t = $('<select id="chbx" multiple="multiple">').appendTo( c.$filters.eq(i) );
					} //if (sel1){
						//t = $('<select>').appendTo( c.$filters.eq(i) );
						//t = $('<select id="s1" multiple="multiple">').appendTo( c.$filters.eq(i) );
					//}
					else {
